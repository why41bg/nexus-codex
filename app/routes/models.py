...
@router.get("/models")
async def list_models(api_key: str = Depends(api_key_auth_dependency), deps: AppDependencies = Depends(get_deps)):
...
async def get_model(model_id: str, api_key: str = Depends(api_key_auth_dependency), deps: AppDependencies = Depends(get_deps)):
