...
interface RowProps {
  acc: Account;
  qs: QuotaState | undefined;
  tick: number;
  onDetail: (acc: Account) => void;
  onEdit: (acc: Account) => void;
  onToggle: (acc: Account) => void;
  onDelete: (acc: Account) => void;
  onFetchQuota: (acc: Account, forceRefresh?: boolean) => void;
}

const AccountDesktopRow = memo(function AccountDesktopRow({
  acc, qs, tick: _tick, onDetail, onEdit, onToggle, onDelete, onFetchQuota,
}: RowProps) {
  const status = getAccountStatus(acc);
  return (
    <tr className="transition-colors hover:bg-gray-50/50 dark:hover:bg-slate-700/50">
      <td className="cursor-pointer px-4 py-3" onClick={() => onDetail(acc)}>
      ...
      <td className="px-4 py-3">
        {!qs && (
          <button
            onClick={(e) => { e.stopPropagation(); onFetchQuota(acc); }}
        ...
        {qs?.data && (
          <div className="flex flex-col gap-1">
            ...
            <button
              onClick={(e) => { e.stopPropagation(); onFetchQuota(acc, true); }}
      ...
      <td className="whitespace-nowrap px-4 py-3 text-right">
        <div className="flex items-center justify-end gap-2">
          <button onClick={(e) => { e.stopPropagation(); onEdit(acc); }} className={subtleBtnClass}>编辑</button>
          <button onClick={(e) => { e.stopPropagation(); onToggle(acc); }} className={acc.enabled ? warningSubtleBtnClass : successSubtleBtnClass}>
            {acc.enabled ? '禁用' : '启用'}
          </button>
          <button onClick={(e) => { e.stopPropagation(); onDelete(acc); }} className={dangerSubtleBtnClass}>删除</button>
        </div>
      </td>
    </tr>
  );
...
const AccountMobileCard = memo(function AccountMobileCard({
  acc, tick: _tick, onDetail, onEdit, onToggle, onDelete,
}: MobileCardProps) {
  const status = getAccountStatus(acc);
  return (
    <div className={`${cardClass} p-4`} onClick={() => onDetail(acc)}>
      <div className="flex items-center gap-2">
        <span className={`inline-block h-2.5 w-2.5 shrink-0 rounded-full ${status.dot}`} />
...
export default function AccountTable({ accounts, loading, onRefresh }: Props) {
  ...
  const handleDelete = useCallback((acc: Account) => setDeleteTarget(acc), []);
