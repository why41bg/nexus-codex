"""Shared pytest fixtures for Nexus Codex tests."""
...
def build_test_app(mock_deps: AppDependencies) -> FastAPI:
    ...
    This is a plain function (not a fixture) so it can be called directly
    from tests that need a custom mock_deps configuration.
    """
    app = FastAPI()
...
def test_app(mock_deps):
    """Create a FastAPI test app with mocked dependencies (fixture wrapper)."""
...
# ─── Helper: build SSE response data ────────────────────────


def sse_text_response(text: str, model: str = TEST_MODEL) -> list[dict]:
    """Build mock SSE response data for a simple text completion.

    The response.completed event has 'output' at the top level (used by
    _extract_text_from_response) and 'response.usage' (used by
