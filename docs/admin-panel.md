...
管理员凭据支持通过 `data/config.json` 的 `adminAuth` 字段持久化。当用户通过管理面板修改密码后，新凭据会写入 `config.json`。优先级规则：环境变量 `ADMIN_USERNAME` / `ADMIN_PASSWORD` 仅在首次启动创建默认配置时生效，之后以 `config.json` 中的值为准。
...
字段说明：`total` 账号总数、`enabled` 已启用、`healthy` 健康、`totalSlots` 总并发槽位数、`activeSlots` 当前使用中的槽位、`availableSlots` 空闲槽位、`disabled` 已禁用、`unhealthy` 不健康、`totalUsage` 总使用次数、`activeSessions` 活跃会话数。
...
PATCH  /api/admin/accounts/:id          → 更新账号配置 { "enabled", "remark", "maxConcurrency", "codexHome" }
DELETE /api/admin/accounts/:id          → 删除账号（有活跃请求时返回 409）
GET    /api/admin/accounts/:id/quota    → 查询指定账号的配额使用情况
POST   /api/admin/accounts/:id/quota/refresh → 强制刷新指定账号的配额缓存
```

> `PATCH` 实际支持更新 4 个字段：`enabled`（启用/禁用）、`remark`（备注）、`maxConcurrency`（最大并发数）、`codexHome`（CODEX_HOME 路径）。
...
PATCH  /api/admin/keys/:keyPrefix → 更新 API Key 配置（名称、模型权限等）
...
```
GET    /api/admin/stream           → SSE 实时事件流（需 Bearer token 认证）
```

管理面板通过 Server-Sent Events 接收实时状态变更通知。支持的事件类型：

- `pool_changed`：账号池状态变化（账号增删、并发槽位变动）
- `health_changed`：账号健康状态变化（包含 `accountId` 和 `healthy` 字段）
...
顶部一排统计卡片，展示账号池整体状况（账号总数、并发槽位使用情况、空闲槽位、不健康、已禁用、总请求数），不同颜色区分语义。下方展示模型白名单管理。

### 3. 账号管理 Tab

表格展示所有账号的详细信息，每行一个账号，包含状态圆点、ID、备注、并发占用（如 `2/3`）、CODEX_HOME 路径、使用次数、最后使用时间、操作按钮。表格上方有状态筛选标签（全部/空闲/使用中/不健康/已禁用）。底部内嵌添加账号表单（支持设置最大并发数）。
...
展示 API Key 列表（脱敏显示），支持创建、编辑（名称、模型权限）和删除。编辑通过模态框进行。

---

## 数据刷新策略

面板通过 SSE（Server-Sent Events）实时接收后端状态变更推送，收到 `pool_changed` 或 `health_changed` 事件后自动刷新数据（500ms debounce 防止请求风暴）。同时在执行任何操作（添加/删除/启用/禁用等）后立即刷新。用户也可以手动触发刷新。
