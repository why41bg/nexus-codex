...
定时对每个账号发送轻量探测，自动下线失效账号：

```typescript
setInterval(async () => {
  for (const entry of pool.entries()) {
    if (entry.activeCount >= entry.maxConcurrency) continue; // 跳过已满载的账号
    try {
      const thread = entry.codex.startThread({ skipGitRepoCheck: true });
      const turn = await thread.run('reply with: ok');
      entry.healthy = turn.finalResponse?.includes('ok') ?? false;
    } catch {
      entry.healthy = false;
    }
  }
...
每个账号支持可配置的并发槽位数（`maxConcurrency`），通过 `activeCount` 计数器实现并发控制。调度策略为最小负载优先：优先选取 `activeCount` 最小的账号，同负载时轮询作为 tie-breaker。全局默认并发数通过环境变量 `DEFAULT_MAX_CONCURRENCY` 配置（默认 1，向后兼容），每个账号可独立覆盖。请求进来时若所有账号的并发槽位均已满载，返回 `429 Too Many Requests`（符合 OpenAI API 的限流语义），由调用方决定是否重试。
